...
## 下一次施工採用 GCT / Global-First Completion

使用者於 2026-09-11 指定「明天也用這一個方法」。下一次實作前，閱讀本專案保存的 [Global-First Completion 技能](docs/methods/gct-v1.0/skill-package/global-first-completion/SKILL.md)；[方法採用記錄](docs/methods/gct-v1.0/ADOPTION.md) 保存來源與適用範圍。

- 先界定當日最小完整交付範圍，再把該範圍的主要資料流、介面、錯誤路徑與必要文件一起接齊；每日一個進度是完整工作包，不限於一個小節點。
- 採用整體實作 → 整體驗證 → 依根因批次修復 → 必要精修 → 最終驗證。廉價語法檢查可隨手執行；沒有新變更或新證據時，不反覆重跑相同完整測試。
- 已授權、可逆且範圍內的工作直接完成。驗證深度按實際失敗影響與契約需求調整，不因抽象風險擴張任務類別或增加程序。
- 完成時確認整個交付流程與必要義務均已處理。測試數、空殼介面及文件覆蓋不能單獨代表完成；仍在範圍內的必要缺口不得自行改列 deferred 以結案。
- GCT 決定施工方式；CRDWS 架構、RGGD 生成語義與本檔 Runtime 不變量繼續適用。使用其他方法時，以本次使用者明確指定的 GCT 執行節奏為準。
