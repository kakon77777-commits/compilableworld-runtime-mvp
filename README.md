...
目前十一個核心交換邊界已有版本化 Schema 檔，放在 [`schemas/`](schemas/)：
...
- `action-behaviors.v0.1.schema.json`：有界 duration、完成模組、並行限制與中斷事件的 Action-scope 複合行為。
...
`action_behaviors.json` 可將一個玩家 verb 綁定到 1–1,000,000 tick 的 authored 行為、唯一完成模組、`one_per_actor` 並行限制，以及移動、受傷或被擊敗三種 bounded 中斷事件。Kernel 會依序產生 `action.scheduled`、`action.started`、`action.completed`／`action.failed`；手動取消與事件中斷則分別留下 `action.cancelled`、`action.interrupted`。完成模組仍只回傳 StateDelta／EventIR，不能直接修改 StateStore。

Gray Crown 的 `search` 是第一個完整切片：排程兩 tick 後由 `exploration.core` 提交搜索計數與 `exploration.searched`。終端可用 `pending` 檢視進度、用 `cancel <action_id>` 取消自己的排程；移動或受到正式戰鬥傷害時會自動中斷。Snapshot 保存精確 tick 與進度，EventLog Replay 可由 private `action.scheduled` 重建尚未結束的 queue。完整規格見 `docs/ACTION_SCOPE_BEHAVIOR_EXECUTION_CONTRACT_zh-TW.md`。
...
- World／Region／Scene／Entity／System 的 scoped StateIR 已可透過 EventIR 跨層轉移；Action-scope 也已有可中斷／取消的 bounded v0.1。兩者都尚未支援地理感知的自動事件路由、自由 guard、resume／補償交易或平行子步驟。
...
- 核心 JSON／CSV Schema 已外部化為 `schemas/` 下的十一份 v0.1 契約；CSV header、scoped StateIR、Action-scope behavior、EveGlyph World IR migration 與人工 mapping validation 已接入，跨檔案引用與其他語意規則仍由 Compiler 驗證。
