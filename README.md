...
- 規則魔法施法系統（同一份 `combat_resolution_system.json` 的 `rule_magic_casting_system`）已實作 MP/FP 資源池（由 MAG/DEX 在編譯期自動推導，五維屬性齊全的實體都會有）與 `cast <法術名>` 指令，目前只接了一個法術（`護盾術`，5 符號，MP40/FP25，temp_HP=施法者MAG×2，優先於真實生命值承受傷害，見 `MagicModule`／`combat_formulas.apply_damage`）作為端到端可用的示範，而非把整個 38 符號組合庫一次全部接完。**刻意未實作**：多交鋒引導詠唱（`cast_time_exchanges>1` 的高階法術，需要在交鋒之間插入「引導中斷判定」，目前交鋒已實作但引導/中斷邏輯還沒有）；完整的異常狀態框架（麻痺/破綻/凍傷/束縛/靜默等 11 種狀態，含持續時間與疊加規則——交鋒回合制已就位，是接這塊的前提，但狀態本身還沒接）；近戰以外招式（`ranged_precision_physical`／`mental_spiritual` 這兩種攻擊類型公式已在來源文件定義好，尚未接線）。這些是該檔案裡份量最大的剩餘部分，留待未來階段。
