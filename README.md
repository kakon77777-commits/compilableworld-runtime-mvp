...
目前版本化交換契約都放在 [`schemas/`](schemas/)，共二十份 schema 檔；catalog 暴露十一個 current authoring/runtime contract，並保留 Action behavior v0.1–v0.7 與 StateIR v0.1 相容來源：
...
- `state-machines.v0.2.schema.json`：World／Region／Scene／Entity／System owner scope、EventIR 選邊與最多 16 個 owner／actor StateStore AND conditions；v0.1 來源仍可編譯為空條件。
...
`state_machines.json` 現在可宣告 World／Region／Scene／Entity／System 五種 owner scope。v0.2 transition 除了 bounded `event_match`，還可用最多 16 個 `when` 條件，以 AND 讀取 machine `owner` 或由 EventIR causation 鏈驗證的 `actor` State Cell；缺值、未知 provenance、非有限數字與錯誤型別都 fail closed。Compiler 會驗證 owner 引用、條件白名單、狀態可達性、終態、事件 payload 白名單、priority 衝突與 `state_machine.core` authority；Runtime 只允許每台機器寫入自己的 `owner::fsm::<state_machine_id>`，並透過 `fsm.transitioned`、`fsm.completed`、`fsm.failed` 與其他模組串接。World、Region、Scene、Entity、System 可同時響應一個正式 EventIR，terminal FSM 也可沿有界 causation 鏈驅動另一台 FSM 或 actor quest。Snapshot、EventLog 與 Replay 保留全部 scoped state。

owner scope 表示狀態所有權、condition owner 與可見性，不會自動把事件限制在某個地區或房間；需要地域路由時，authoring 必須用事件型別與 `event_match` 明確表達。OR／NOT 群組、自由文字 guard、任意 effects/reward 與 Runtime random sampling 仍不在此契約內；Action-scope 複合行為則由獨立契約控制。完整規格見 `docs/SCOPED_STATE_IR_EXECUTION_CONTRACT_zh-TW.md`。
...
- 核心 JSON／CSV Schema 已外部化為 `schemas/` 下二十份契約檔（含 Action behavior v0.1–v0.7 與 StateIR v0.1 相容契約）；CSV header、scoped StateIR、Action-scope behavior、EveGlyph World IR migration 與人工 mapping validation 已接入，跨檔案引用與其他語意規則仍由 Compiler 驗證。
