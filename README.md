...
目前版本化交換契約都放在 [`schemas/`](schemas/)，共二十四份 schema 檔；catalog 暴露十一個 current authoring/runtime contract，並保留 Action behavior v0.1–v0.7 與 StateIR v0.1–v0.6 相容來源：
...
- `state-machines.v0.6.schema.json`：在 v0.5 single-active-leaf hierarchy 上加入 bounded `all`／`any`／`not` condition expression、三值 fail-closed 與 group／node／leaf budget；Compiler 仍驗證 hierarchy、timer、reaction DAG 與 condition ID，v0.1–v0.5 來源仍可編譯。
