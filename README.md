...
目前十一個核心交換邊界已有版本化 Schema，放在 [`schemas/`](schemas/)；Action behavior 同時保留 v0.1–v0.5，因此實際共有十五份檔案：
...
- `action-behaviors.v0.5.schema.json`：有界 sequential phases、非遞迴 primitive child Actions、phase-entry conditions、fixed-interval retry/deadline、完成模組、並行限制與中斷事件；v0.1 單階段、v0.2 sequential、v0.3 condition-gated 與 v0.4 retry-bounded 來源仍可編譯。
...
`state_machines.json` 現在可宣告 World／Region／Scene／Entity／System 五種 owner scope。Compiler 會驗證 owner 引用、狀態可達性、終態、事件 payload 白名單、priority 衝突與 `state_machine.core` authority；Runtime 只允許每台機器寫入自己的 `owner::fsm::<state_machine_id>`，並透過 `fsm.transitioned`、`fsm.completed`、`fsm.failed` 與其他模組串接。World、Region、Scene、Entity、System 可同時響應一個正式 EventIR，terminal FSM 也可沿有界 causation 鏈驅動另一台 FSM 或 actor quest。Snapshot、EventLog 與 Replay 保留全部 scoped state。

owner scope 在 v0.1 表示狀態所有權與可見性，不會自動把事件限制在某個地區或房間；需要地域路由時，authoring 必須用事件型別與 `event_match` 明確表達。自由文字 guard、任意 effects/reward 與 Runtime random sampling 仍不在此契約內；Action-scope 複合行為則由獨立契約控制。完整規格見 `docs/SCOPED_STATE_IR_EXECUTION_CONTRACT_zh-TW.md`。

### Action-scope 複合行為

`action_behaviors.json` 可將一個玩家 verb 綁定到 2–64 個固定順序 phase，每個 phase 有獨立名稱與 1–1,000,000 tick duration，原始總長仍不得超過 1,000,000 tick。v0.5 的非 final phase 可放一個已編譯的 primitive child Action，依 phase 順序執行且成功後不因 gate retry 重跑；child 只能繼承父 actor，使用白名單 verb／module／target／args，不能遞迴 authored behavior。非首 phase仍可宣告最多 16 個 AND gate，以及可選的 1–16 次 fixed-interval retry 與 authored deadline。Kernel 將 child Module 的 StateDelta／EventIR、child lifecycle 與父 progress／retry／failure 原子寫入同一 EventLog batch；最後效果仍由唯一 completion module 提交。

Gray Crown 的 `search` 是第一個完整切片：第一 tick 先以 `survey.room` child 執行 `look`，再由 `actor_alive` 決定是否進入「仔細檢查線索」；不成立時最多以一 tick 間隔 retry 兩次，但已完成 child 不重跑。終端會顯示 child／phase／retry 進度，也可用 `pending` 檢視或取消；player projection 只顯示 step／condition ID 與安全的 retry attempt/deadline，不公開 child args、condition namespace、operator 或比較值。Snapshot v0.4 與 EventLog Replay 都可重建 due tick、retry state 與 completed child prefix。完整規格見 `docs/ACTION_SCOPE_BEHAVIOR_EXECUTION_CONTRACT_zh-TW.md`。

### Snapshot 與排程恢復

`save_snapshot` 使用 `compilableworld.snapshot/v0.4`，除了 Runtime State、動態玩家、生成 profile、目前 tick 與排程 Action，也保存 pending Action 的 retry attempt、first failure、next retry、deadline 與 authored-prefix `completed_steps`。載入時會驗證 Snapshot 格式、版本、世界 ID、世界版本及 Action runtime state；舊 `v0.1` 以空排程遷移，`v0.2`／`v0.3` 的 pending Action 以空 child progress 遷移。
...
- World／Region／Scene／Entity／System 的 scoped StateIR 已可透過 EventIR 跨層轉移；Action-scope 也已有可中斷／取消的 bounded v0.5 sequential phase checkpoints、primitive child sequence、phase-entry AND gates 與 fixed-interval retry/deadline。兩者都尚未支援地理感知的自動事件路由、自由 guard、if/else branching、resume／補償交易、自由 backoff/jitter、遞迴／任意 sequence graph 或 parallel／join。
...
- 核心 JSON／CSV Schema 已外部化為 `schemas/` 下的十一個交換邊界（十五份檔案，含 Action behavior v0.1–v0.4 相容契約）；CSV header、scoped StateIR、Action-scope behavior、EveGlyph World IR migration 與人工 mapping validation 已接入，跨檔案引用與其他語意規則仍由 Compiler 驗證。
