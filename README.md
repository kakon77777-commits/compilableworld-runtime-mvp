...
目前十一個核心交換邊界已有版本化 Schema，放在 [`schemas/`](schemas/)；Action behavior 同時保留 v0.1 與 v0.2，因此實際共有十二份檔案：
...
- `action-behaviors.v0.2.schema.json`：有界 sequential phases、完成模組、並行限制與中斷事件的 Action-scope 複合行為；v0.1 單階段來源仍可編譯。
...
`action_behaviors.json` 可將一個玩家 verb 綁定到 2–64 個固定順序 phase，每個 phase 有獨立名稱與 1–1,000,000 tick duration，總長仍不得超過 1,000,000 tick。Kernel 在非最終 phase 邊界產生 `action.progressed`，最後才依序產生 `action.started`、`action.completed`／`action.failed`；手動取消與事件中斷則分別留下 `action.cancelled`、`action.interrupted`。每個 behavior 仍只有唯一完成模組與 `one_per_actor` 並行限制，完成模組只能回傳 StateDelta／EventIR，不能直接修改 StateStore。

Gray Crown 的 `search` 是第一個完整切片：第一 tick 完成「觀察搜索範圍」並發出 checkpoint，第二 tick 才由 `exploration.core` 提交搜索計數與 `exploration.searched`。終端會顯示 phase 進度，也可用 `pending` 檢視或用 `cancel <action_id>` 取消自己的排程；移動或受到正式戰鬥傷害時會自動中斷。Snapshot 保存精確 tick／phase，EventLog Replay 可由 private lifecycle 重建尚未結束的 queue。完整規格見 `docs/ACTION_SCOPE_BEHAVIOR_EXECUTION_CONTRACT_zh-TW.md`。
...
- World／Region／Scene／Entity／System 的 scoped StateIR 已可透過 EventIR 跨層轉移；Action-scope 也已有可中斷／取消的 bounded v0.2 sequential phase checkpoints。兩者都尚未支援地理感知的自動事件路由、自由 guard、resume／補償交易、conditional／retry，或會逐步執行任意子 Action 的 parallel／sequence graph。
...
- 核心 JSON／CSV Schema 已外部化為 `schemas/` 下的十一個交換邊界（十二份檔案，含 Action behavior v0.1 相容契約）；CSV header、scoped StateIR、Action-scope behavior、EveGlyph World IR migration 與人工 mapping validation 已接入，跨檔案引用與其他語意規則仍由 Compiler 驗證。
