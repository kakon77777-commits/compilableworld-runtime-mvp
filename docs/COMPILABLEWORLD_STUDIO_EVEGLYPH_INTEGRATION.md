...
正式主線現在把交換邊界外化成 [`schemas/`](../schemas/) 下二十三份 schema；catalog 暴露十一個 current authoring/runtime contract，Action behavior 同時保留 v0.1–v0.7，StateIR 保留 v0.1–v0.5：
...
- `state-machines.v0.5.schema.json`：五種 owner scope、single-active-leaf compound hierarchy、EventIR／leaf timer 選邊與 owner／actor StateStore AND conditions；Compiler 驗證 direct parent／initial child、16 層 hierarchy、resolved leaf、leaf-over-ancestor specificity 與 64 層 reaction DAG；`v0.1`–`v0.4` 檔案保留為來源相容邊界。
