...
- `action_behaviors.json`：版本化的 Action-scope 行為來源；v0.5 可宣告固定順序 phases、非遞迴 primitive child Actions、bounded phase-entry conditions 與 fixed-interval retry/deadline。Studio authoring overview 可唯讀顯示完整 child／條件／policy，player/runtime pending projection 只顯示 child step ID／完成數、condition ID、安全的 retry attempt/deadline 與目前 phase/progress；正式修改仍回到 authoring source。
...
正式主線現在把十一個核心交換邊界外化成 [`schemas/`](../schemas/) 下的 JSON／CSV Schema；Action behavior 同時保留 v0.1–v0.5，所以共有十五份檔案：
...
- `action-behaviors.v0.5.schema.json`：Action-scope sequential phases、非遞迴 primitive child Actions、bounded conditions、fixed-interval retry/deadline、完成模組、並行限制與中斷事件契約；`v0.1`–`v0.4` 檔案保留為來源相容邊界。
