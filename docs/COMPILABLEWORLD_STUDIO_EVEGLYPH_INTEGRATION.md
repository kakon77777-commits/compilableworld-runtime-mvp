...
- `state_machines.json`：版本化的 World／Region／Scene／Entity／System StateIR；v0.5 在 bounded EventIR／timer／reaction DAG 上加入最多 16 層、單一 active leaf 的 compound hierarchy。Studio overview 可唯讀顯示 parent／initial-child maps、initial／current path、authored target、resolved target leaf、entry tick 與 pending timer countdown。
...
v0.4／v0.5 的非終態 `fsm.transitioned` 是 reviewed `state_machines.json` 的直接 StateIR 契約。現行 `studio-compile` 的 state-machine overlay 仍只產生 `target: quest`，因此 mapping validator 會以 `nonterminal_stateir_only` 拒絕把這個事件偽裝成 Quest overlay；EveGlyph 可先顯示與編輯候選，但要由 StateIR write-back／Compiler 完成來源、hierarchy、reaction cycle 與 depth 審查。
...
正式主線現在把交換邊界外化成 [`schemas/`](../schemas/) 下二十四份 schema；catalog 暴露十一個 current authoring/runtime contract，Action behavior 同時保留 v0.1–v0.7，StateIR 保留 v0.1–v0.6：
...
- `state-machines.v0.6.schema.json`：五種 owner scope、single-active-leaf compound hierarchy、EventIR／leaf timer 選邊與 owner／actor StateStore bounded `all`／`any`／`not` condition expression；Compiler 驗證 group／node／leaf budget、direct parent／initial child、16 層 hierarchy、resolved leaf、leaf-over-ancestor specificity 與 64 層 reaction DAG；`v0.1`–`v0.5` 檔案保留為來源相容邊界。
