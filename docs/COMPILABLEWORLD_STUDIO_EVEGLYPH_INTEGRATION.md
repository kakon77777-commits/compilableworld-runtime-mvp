...
- `action_behaviors.json`：版本化的 Action-scope 行為來源；v0.6 可宣告固定順序 phases、priority conditional child branches、implicit linear rejoin、非遞迴 primitive child Actions、bounded phase-entry conditions 與 fixed-interval retry/deadline。Studio authoring overview 可唯讀顯示完整 branch／child／條件／policy，player/runtime pending projection 只顯示 branch ID／priority／condition IDs、child step ID／完成數、安全的 retry attempt/deadline 與目前 phase/progress；正式修改仍回到 authoring source。
...
正式主線現在把交換邊界外化成 [`schemas/`](../schemas/) 下十八份 JSON／CSV Schema；Action behavior 同時保留 v0.1–v0.6：
...
- `action-behaviors.v0.6.schema.json`：Action-scope sequential phases、priority conditional child branches、implicit linear rejoin、非遞迴 primitive child Actions、bounded conditions、fixed-interval retry/deadline、完成模組、並行限制與中斷事件契約；`v0.1`–`v0.5` 檔案保留為來源相容邊界。
