...
- `action_behaviors.json`：版本化的 Action-scope 行為來源；Studio overview 可唯讀顯示 duration、completion module、中斷條件與目前 pending progress，正式修改仍回到 authoring source。
...
正式主線現在把十一個核心交換邊界外化成 [`schemas/`](../schemas/) 下的 JSON／CSV Schema：
...
- `action-behaviors.v0.1.schema.json`：Action-scope duration、完成模組、並行限制與中斷事件契約。
