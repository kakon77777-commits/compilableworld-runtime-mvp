...
- `action_behaviors.json`：版本化的 Action-scope 行為來源；v0.4 可宣告固定順序 phases、bounded phase-entry conditions 與 fixed-interval retry/deadline。Studio authoring overview 可唯讀顯示完整條件與 policy，player/runtime pending projection 只顯示 condition ID、安全的 retry attempt/deadline 與目前 phase/progress；正式修改仍回到 authoring source。
...
正式主線現在把十一個核心交換邊界外化成 [`schemas/`](../schemas/) 下的 JSON／CSV Schema；Action behavior 同時保留 v0.1、v0.2、v0.3 與 v0.4，所以共有十四份檔案：
...
- `action-behaviors.v0.4.schema.json`：Action-scope sequential phases、bounded conditions、fixed-interval retry/deadline、完成模組、並行限制與中斷事件契約；`v0.1`／`v0.2`／`v0.3` 檔案保留為來源相容邊界。
