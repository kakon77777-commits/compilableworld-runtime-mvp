...
- **Verification:** `PYTHONPATH=src python -m unittest discover -s tests` — **285/285 passed**
...
| Action-scope state machines | Implemented | v0.2 bounded sequential phase checkpoints, exact-tick `action.progressed`, v0.1 source compatibility, `one_per_actor`, lifecycle EventIR, cancellation, interruption, Snapshot and pending Replay; child Action Graph, conditional, retry, resume, compensation, parallel steps and arbitrary guards remain pending; see `docs/ACTION_SCOPE_BEHAVIOR_EXECUTION_CONTRACT_zh-TW.md` |
