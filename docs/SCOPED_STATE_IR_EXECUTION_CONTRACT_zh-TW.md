...
- 本 StateIR 契約內的 Action-scope 狀態；可中斷／取消的 bounded 行為已由獨立 `action-behaviors/v0.1` 契約提供，但 resume／補償／平行子步驟仍未包含；
...
驗證基線由 `tests/test_scoped_state_machine.py` 覆蓋五種 owner、跨層 chaining、terminal FSM 到 Quest、Snapshot、Replay、Studio projection、legacy seed 相容與 reaction rollback；完整測試為 281/281。
