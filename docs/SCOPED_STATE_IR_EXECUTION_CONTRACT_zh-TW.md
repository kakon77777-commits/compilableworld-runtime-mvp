...
- 本 StateIR 契約內的 Action-scope 狀態；可中斷／取消、sequential checkpoint、非遞迴 primitive child sequence、bounded phase gates 與 fixed-interval retry/deadline 已由獨立 `action-behaviors/v0.5` 契約提供，但任意／遞迴 Action Graph、branching、resume／補償／平行子步驟仍未包含；
...
驗證基線由 `tests/test_scoped_state_machine.py` 覆蓋五種 owner、跨層 chaining、terminal FSM 到 Quest、Snapshot、Replay、Studio projection、legacy seed 相容與 reaction rollback；完整測試為 305/305。
