...
class EntityTransactionTests(unittest.TestCase):
    ...
    def test_plain_delayed_creation_preserves_action_lifecycle(self) -> None:
        runtime = self.runtime()
        action = ActionIR("parent.001", "spawn")
        self.assertEqual(runtime.submit(action, delay=2).status.value, "scheduled")
        self.assertFalse(runtime.registry.contains("child.001"))

        receipts = runtime.advance(2)
        ...
        self.assertEqual(runtime.scheduler.entries(), [])
        self.assertNotIn(action.action_id, runtime.action_runtime)

    def test_plain_delayed_rejection_preserves_action_lifecycle(self) -> None:
        runtime = self.runtime(UndeclaredSpawnModule())
        action = ActionIR("parent.001", "spawn")
        runtime.submit(action, delay=2)
    ...
    def test_reactions_follow_the_complete_entity_commit_batch(self) -> None:
        runtime = self.runtime()
        observer = SpawnModule()
        observer.contract = ModuleContract(
            module_id="test.observer", version="0.1.0", layer="runtime",
            actions=[], events=["test.observed"], read=[], write=["lineage.*"],
        )
        runtime.register_module(observer)
        observed = []
        runtime.events.subscribe("*", lambda event: observed.append(event.event_id))

        def react(event: EventIR) -> None:
            if event.source == "test.spawn":
                runtime.commit_reaction(
                    observer,
                    [StateDelta("child.001", "lineage", "observed", "set", True)],
                    [EventIR("test.observed", "test.observer", {}, causation_id=event.event_id)],
                )

        runtime.events.subscribe("state.committed", react)
        receipt = runtime.submit(ActionIR("parent.001", "spawn"))

        self.assertEqual(receipt.status.value, "completed")
        self.assertTrue(runtime.state.get("child.001", "lineage", "observed"))
        self.assertEqual(observed, [event.event_id for event in runtime.event_log.events])
        self.assertEqual(
            [event.event_type for event in runtime.event_log.events],
            ["state.committed", "entity.committed", "test.entity_spawned",
             "state.committed", "test.observed"],
        )
